...
import { ArrowLeft, BellRing, CalendarClock, Pin, Send } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import type { NoticeCommentRow, NoticeDetailRow } from "@/lib/types";

export function NoticeDetail({ notice }: { notice: NoticeDetailRow }) {
  const [comments, setComments] = useState(notice.comments);
  const [commentCount, setCommentCount] = useState(notice.commentCount);
  ...
  return (
    <div className="mx-auto w-full max-w-[1280px] space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#0d1b3d]">공지사항 조회</h1>
        </div>
      ...
      <article className="rounded-lg border border-[#dbe3ee] bg-white p-6 shadow-[0_10px_34px_rgba(20,35,65,0.06)]">
        ...
        <h2 className="mt-3 text-2xl font-bold leading-8 text-[#0d1b3d]">{notice.title}</h2>

        {notice.popupEnabled ? (
          <div className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-[#66748a]">
            <CalendarClock className="size-3.5" />
            {formatPopupRange(notice.popupStartsAt, notice.popupEndsAt)}
          </div>
        ) : null}

        <div className="mt-5 min-h-[320px] whitespace-pre-wrap rounded-lg border border-[#dbe4ef] bg-[#f8fbff] p-6 text-sm leading-7 text-[#22304f]">
          {notice.content}
        </div>
      </article>

      <section className="rounded-lg border border-[#dbe3ee] bg-white p-4 shadow-[0_6px_20px_rgba(20,35,65,0.05)]">
        <h3 className="text-sm font-bold text-[#0d1b3d]">댓글 {commentCount.toLocaleString()}</h3>
...
function CommentItem({ comment }: { comment: NoticeCommentRow }) {
  return (
